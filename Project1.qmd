...
Looking at the intercept and slope of my models gives interesting results. The slopes of the nurse communication and doctor communication variables are positive which is expected as when either score increases the overall rating increases, but the intercept doesn't make much sense in the context of our hospital scores. Rounding our intercept it is around -29.144 and the lowest score a hospital can get is zero. So, if a hospital had a zero on doctor communication and nurse communication it would be expected to have a negative score from our model, which isn't possible. Our 2nd model with all variables similarly, has a negative intercept which doesn't fit our context, along with having a positive slope for most of the variables but our communication about medicines variable is a bit of an outlier. It has a negative slope, meaning holding the other variables constant, in this model as our communication about medicines score increases the overall hospital rating is actually expected to decrease.
...
I made a 2 scatter plots for my models, using ggplot.
...
geom_histogram() +
   labs(title = ".resid Historgram all variables Model") 
...
Here I computed the values of my rmse by taking each of the residual values for the three models, squaring the residuals,taking the mean value and then square rooting.
...
I made a table of the resulting Rmse's based on each model. Since the smaller the Rmse the closer the model is to what it's trying to predict the "best" model is the communication model, followed by the all variables model, then the null model. It makes sense our null model predicts the worst as any values under its predicted rating of 87 result in a positive residual and the score is out of 100, so 87 shouldn't always be expected.
